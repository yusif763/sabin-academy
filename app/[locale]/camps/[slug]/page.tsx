...
export default async function CampDetailPage({ 
  ...
  const trans = camp.translations.find((t: any) => t.locale === params.locale)
  if (!trans) notFound()

  const duration = Math.ceil((new Date(camp.endDate).getTime() - new Date(camp.startDate).getTime()) / (1000 * 60 * 60 * 24))
          ...
          <div className="max-w-4xl">
            ...
            <div className="flex flex-wrap gap-6 text-white/90">
              ...
              <div className="flex items-center">
                ...
                <span>{new Date(camp.startDate).toLocaleDateString(params.locale)} - {new Date(camp.endDate).toLocaleDateString(params.locale)}</span>
              </div>
              <div className="flex items-center">
                  ...
                  <div className="space-y-3">
                    {trans.activities.map((activity: string, index: number) => (
                      <div key={index} className="flex items-start bg-primary-50 p-4 rounded-lg">
                        <div className="w-8 h-8 bg-primary-500 text-white rounded-full flex items-center justify-center font-bold mr-4 flex-shrink-0">
                          {index + 1}
                        </div>
